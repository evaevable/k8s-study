...
> - 前置知识：第 2 章（容器重启 ≠ Pod 重启）、第 3 章（节点失联与驱逐）、第 5 章（就绪探针是滚动更新的刹车）、第 6 章（Pod 不 Ready 会被移出后端）
> - 读完你应该能回答四个问题：
>   1. **K8s 到底能修哪些故障，不能修哪些？**"自愈"这个词的边界在哪？
>   2. 三种探针（`liveness` / `readiness` / `startup`）**分别触发什么动作**？
>   3. 怎么用**一个探针配置错误**把整个服务搞挂？（这是最容易自己制造的全站故障）
>   4. 进程还在跑但服务已经不响应了，谁来救？为什么说 **`liveness` 是个危险工具**？
...
## 【积木 11-2】`restartPolicy` 与 CrashLoopBackOff 的由来
...
2. **`livenessProbe` 失败会杀死容器**——这是**破坏性**的，所以它必须谨慎设计（积木 11-5 会讲配错的后果）。
...
| 应用本身很健壮，从不假死 | **都不配也行** | **见积木 11-5 的观点** |
...
**解法**：用 `startupProbe`（积木 11-4），而不是把 `initialDelaySeconds` 调大。
...
## 【积木 11-6】完整推演：一次探针配错如何造成全站故障

把积木 11-5 的事故一完整走一遍，看看每一个环节：
...
## 【积木 11-7】自愈能力的完整边界：K8s 能修什么、不能修什么
...
1. 自愈的三个层次分别是谁在修、修什么、Pod IP 会不会变？（积木 11-1）
2. `RESTARTS` 统计的是什么？为什么 Pod 重建后它是 0？（积木 11-1）
3. 谁负责重启容器？`restartPolicy` 是 Pod 级还是容器级？（积木 11-2）
4. 指数退避的起始值和上限分别是多少？为什么要退避？（积木 11-2）
5. `CrashLoopBackOff` 是什么意思？排查它的第一把钥匙是什么命令？（积木 11-2）
6. 三种探针失败时**分别触发什么动作**？哪个是破坏性的？（积木 11-3）
7. 为什么说"不是所有服务都需要 `livenessProbe`"？（积木 11-3）
8. `tcpSocket` 探针有什么陷阱？（积木 11-4）
9. 一个 liveness 配置是 `initialDelaySeconds: 10, periodSeconds: 10, failureThreshold: 3`，从假死到被杀最久要多久？（积木 11-4）
10. `startupProbe` 的"最大允许启动时间"怎么算？它成功后会发生什么？（积木 11-4）
11. **为什么 `liveness` 探针检查外部依赖会造成"连锁故障"？**完整推演一遍。（积木 11-5、11-6）
12. `initialDelaySeconds` 太短会导致什么死循环？正确解法是什么？（积木 11-5）
13. `liveness` 和 `readiness` 用同一个端点有什么问题？（积木 11-5）
14. 判断一个检查该不该放进 `liveness` 的实用问句是什么？（积木 11-5）
15. "降级（degraded）"状态是什么？它比"直接重启"好在哪？（积木 11-5、11-6）
16. 列出至少五种 **K8s 修不了**的故障，并各说出一个应对手段。（积木 11-7）
17. 为什么 `readiness` 探针失败不会增加 `RESTARTS`？（积木 11-8）

### 下一章预告

**第 12 章：弹性伸缩 —— HPA、VPA 与 Cluster Autoscaler**

CloudNote 现在能自愈了，但它还不会"长大"。第 1 章我们埋的那个"人肉扩容"地狱还没解决：

> **每天晚上 20:00 是笔记同步高峰，`api` 需要从 2 个副本扩到 10 个。**难道要写个定时脚本吗？
>
> `HPA` 是怎么"看见"负载的？它凭什么决定"该扩到几个"？（提示：它不看你日志里的 QPS）
>
> 为什么 `HPA` 经常"扩不上去"？**扩容了但 Pod 一直 Pending** 是什么原因？
>
> `VPA` 和 `HPA` 能一起用吗？（答案有点尴尬）`Cluster Autoscaler` 又解决了什么？
