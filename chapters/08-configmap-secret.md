...
| 你的需求 | 用什么 |
|---|---|
| 一份配置文件，能接受改完重启 Pod | **`subPath`**（干净，不污染目录） |
| 一份配置文件，希望改完自动更新 | **目录挂载**（挂到 `conf.d` 这类专用子目录，别挂父目录） |
| 多个文件，希望自动更新 | **目录挂载 + 专门的挂载点目录** |

> **一句话记忆**：**`subPath` 的代价就是"永不更新"。**如果你要"自动更新"，就别用 `subPath`；如果你用了 `subPath`，就必须配合滚动重启（积木 8-5 的四种方案之一）。
...
4. **`subPath` 挂载的文件永不更新**（设计如此）；**目录挂载是覆盖而非合并**。这两个坑造成的故障，比这一章其他内容加起来都多。
...
10. `subPath` 挂载为什么永不更新？什么场景适合用它？（积木 8-6）
