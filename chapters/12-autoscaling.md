...
> - 前置知识：第 5 章（Deployment 与副本数）、第 10 章（requests 是 HPA 的地基）、第 11 章（就绪探针影响指标计算）
> - 读完你应该能回答四个问题：
>   1. `HPA` 是怎么"看见"负载的？**为什么它其实不看 QPS？**
>   2. **为什么 `requests` 写错会让 `HPA` 完全失灵？**（这是全章最关键的一点）
>   3. `HPA` 经常"扩不上去"的五个原因分别是什么？**扩容了但 Pod 一直 `Pending` 又是为什么？**
>   4. `VPA` 和 `HPA` 能一起用吗？（答案有点尴尬）

第 1 章我们埋的第一个地狱就是**人肉扩容**：
...
## 【积木 12-2】HPA 的第一原理：它不看 QPS，它看"比例"
...
## 【积木 12-4】三种指标源：从 CPU 到"队列里积压了多少个任务"
...
| **2** | 容器**没写 `requests`** | `kubectl describe hpa` 里有 `missing request for cpu` | 补上 requests（积木 12-2） |
...
## 【积木 12-7】Cluster Autoscaler：当节点不够时
...
>
> **记住这个时间量级，因为它是架构决策的依据**（回到积木 12-1 那句话：**自动伸缩应对趋势，不应对尖峰**）。
...
## 【积木 12-8】VPA：纵向伸缩，以及它和 HPA 的尴尬关系
...
| **`Recreate`** | 修改已有 Pod 的 requests | ⚠️ **会驱逐并重建 Pod**——**这是 VPA 的最大代价** |
...
**官方立场是"谨慎"**：因为 VPA 改 requests 会**改变 HPA 的分母**（积木 12-2），两个控制器在互相影响对方的输入。
...
>
> 因为自动调节的代价（重建 Pod）和风险（与 HPA 冲突）都不小，而"给建议"这个用法的收益（把 requests 调准）恰恰是 HPA 能否工作的**前提**（积木 12-2）。**两者刚好接上。**

---

## 【积木 12-9】CloudNote 的完整弹性伸缩配置
...
**为什么这个必须用自定义指标**：回到积木 12-4 那个推演——**worker 等 IO 时 CPU 只有 5%，按 CPU 扩缩它永远不动，而队列已经积压了 10000 个任务。**
...
**两者差得多，就说明你的 requests 没设准** —— 而 `requests` 不准，HPA 就算不对（积木 12-2）。
...
> ⚠️ **注意**：`metrics-server` 是集群级组件，清理脚本**不会**删除它（属于整个集群，不属于本章实验）。
...
1. 弹性的三个层次分别管什么？它们之间的依赖链是什么？（积木 12-1）
2. 为什么"自动伸缩应对趋势、不应对尖峰"？完整扩缩链路大概要多久？（积木 12-1）
3. **写出 HPA 的期望副本数公式**，并用"2 副本、当前 70%、目标 50%"算一遍。（积木 12-2）
4. **HPA 说的"CPU 利用率 70%"，分母是什么？**不是 limits 吗？（积木 12-2）
5. `requests` 写得太小和太大，分别会让 HPA 怎么失灵？（积木 12-2）
6. HPA 的"容差（tolerance）"默认是多少？它是干什么用的？（积木 12-2）
7. HPA 工作的三个前置条件是什么？缺了各会怎样？（积木 12-3）
8. `kubectl get hpa` 的 `TARGETS` 显示 `<unknown>/50%` 意味着什么？（积木 12-3、12-6）
9. **为什么 `worker` 这类 IO 密集型服务不能用 CPU 做扩缩指标？**该用什么？（积木 12-4）
10. HPA 扩容和缩容的默认行为有什么不对称？为什么这样设计？（积木 12-5）
11. 为什么"缩容要做得比扩容更慢"？两者的风险怎么不对称？（积木 12-5）
12. 为什么扩容也要限速（`scaleUp` 的 policies）？（积木 12-5）
13. HPA 扩不上去的五个原因分别是什么？各怎么确认？（积木 12-6）
14. "扩了但 Pod 一直 Pending"说明什么？该找谁解决？（积木 12-6、12-7）
15. Cluster Autoscaler 缩容有哪五个条件？为什么要这么严格？（积木 12-7）
16. VPA 的三种 `updateMode` 分别做什么？哪个代价最大？（积木 12-8）
17. VPA 和 HPA 能在同一指标上一起用吗？为什么？（积木 12-8）
18. **VPA 最有价值的用法是什么？**它和 HPA 怎么接上？（积木 12-8）

### 下一章预告

**第 13 章：工作负载全景 —— StatefulSet、DaemonSet、Job 与 CronJob**

`api`、`web`、`worker` 都是无状态的，用 Deployment 就够了。但 CloudNote 里还有两个东西一直没解决：

> **`postgres` 怎么办？**它有状态——**每个副本需要自己的存储、自己的身份、有启动顺序**。用 Deployment 管它，所有副本会抢同一个 PVC（第 9 章埋的伏笔）。
>
> 而且数据库需要**稳定的网络身份**：主库永远是 `postgres-0`，从库永远是 `postgres-1`，**名字和顺序不能乱**（第 6 章讲的 headless Service 终于要派上用场）。
>
> 还有两个需求也没解决：**每个节点上都要跑一个的日志采集器**（Deployment 做不到"每节点一个"）、**跑完就结束的数据迁移任务**（不能一直重启）。

这一章会讲清 **StatefulSet 的三个"稳定"保证**（稳定身份 / 稳定存储 / 有序启停）、**DaemonSet 的适用场景**、以及 **Job 与 CronJob 怎么把"一次性动作"塞进声明式框架**（第 4 章埋的那个问题终于有答案了）。
