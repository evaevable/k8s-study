...
> - 前置知识：第 1 章（尤其要记得"容器是被管对象"和"调和循环"）
> - 读完你应该能回答四个问题：
>   1. 既然真正跑东西的是容器，为什么 K8s 的最小单位是 **Pod** 而不是容器？
>   2. 一个 Pod 里的多个容器，是怎么"共享"网络和存储的？
>   3. init 容器和 sidecar 分别解决什么问题？
>   4. Pod 从被创建到被销毁，中间经历了哪几个阶段？
...
## 【积木 2-3】解剖一个 Pod：那个你看不见的 pause 容器
...
> 顺带说，`wait-for-dependency` 这类逻辑，在纯 K8s 里也可以由**探针 + 重启**兜底（失败了就重启重试），但用 init 容器更明确、日志更清晰、也不会让主容器反复崩溃。第 11 章讲探针时我们会对比这两种思路。

---

## 【积木 2-7】Pod 的生命周期：从 Pending 到 Terminating
...
## 【积木 2-8】动手：写一个多容器 Pod，亲手验证"共享"

理论讲够了，跑一遍。

> 需要先按第 1 章【积木 1-10】起一个集群。没起也没关系，看命令和输出也能懂。
...
## 【积木 2-9】对应到 CloudNote：api 的 Pod 长什么样
...
1. 为什么 K8s 的最小调度单位是 Pod 而不是容器？举出至少两个理由。（积木 2-1）
2. pause 容器的唯一作用是什么？如果没有它，容器重启会发生什么严重后果？（积木 2-3）
3. 同一个 Pod 里两个容器都想监听 8080 端口，会发生什么？为什么？（积木 2-4）
4. Sidecar、Ambassador、Adapter 三种模式各自解决什么问题？用一句话说清区别。（积木 2-5）
5. init 容器和主容器在"执行顺序"和"生命周期"上有哪三点本质区别？（积木 2-6）
6. 一个 Pod 有 3 个主容器和 1 个 init 容器，调度时资源需求怎么算？（积木 2-6）
7. 为什么说 "Pod IP 是稳定的，但 Pod 是易失的"？这两句话矛盾吗？（积木 2-3、2-7）
8. 你手动 `kubectl edit` 改了一个由 Deployment 管理的 Pod，为什么改动很快就"消失"了？（积木 2-7）
9. `emptyDir` 的数据什么时候会丢？什么场景适合用它？（积木 2-9）

### 下一章预告

**第 3 章：集群的解剖学 —— 控制平面与工作节点**

第 1 章我们画了一张集群全景图，但有一批关键问题还没回答：

> API Server 凭什么能扛住整个集群的读写？etcd 为什么必须是奇数个节点？Scheduler 到底是怎么"打分"的？kubelet 为什么每次都要去做一次"体检"？为什么 K8s 的组件几乎全都能挂掉，但 etcd 不能？

这一章我们把这台"分布式操作系统"大卸八块，逐个零件看清楚。看完你就能理解：**为什么 K8s 会说自己是"面向故障设计"的。**
