...
>
> - 建议用时：40 分钟
> - 前置知识：会基本的 Linux 命令，听说过 Docker（不会也没关系）
> - 读完你应该能回答四个问题：
>   1. 已经有 Docker 了，为什么还需要 Kubernetes？
>   2. Kubernetes 到底在"管"什么？
>   3. 「声明式」「期望状态」「控制器循环」这三个词是什么意思？
>   4. 敲下 `kubectl apply` 之后，集群里到底发生了什么？
...
## 【积木 1-4】Kubernetes 是什么：一句话定义 + 一个类比
...
## 【积木 1-7】一次 `kubectl apply` 之后，30 秒内发生了什么
...
下面是全书会出现的对象。**现在不要背**，只需要扫一眼，知道"哦，有这么个东西，它属于哪一类"。
...
> 案例的完整 YAML 清单会随章节逐步添加到本仓库的 `cases/cloudnote/` 目录下。到第 14 章时，你会拿到一套可以直接 `kubectl apply -f` 跑起来的完整配置。

---

## 【积木 1-10】动手：三种零成本起一个集群
...
第 3 个命令很值得跑一下。你会看到一长串资源类型——**这个列表就是 K8s 的"对象字典"**，也是我们后面要逐个攻克的清单。
...
1. 用一个比喻解释"容器"和"虚拟机"的区别。（积木 1-2）
2. 为什么说容器不是"轻量虚拟机"？它到底用了 Linux 的哪两套机制？（积木 1-2）
3. "命令式"和"声明式"的区别是什么？为什么 K8s 选声明式？（积木 1-3）
4. 调和循环在做什么？它为什么能实现"自愈"？（积木 1-3、1-6）
5. 一次 `kubectl apply` 后，是**谁**把 Pod 分配到节点上的？它依据什么？（积木 1-7）
6. 为什么 `kubectl get pods` 能看到最新状态？数据从哪来？（积木 1-7）
7. Deployment 为什么不直接建 Pod，而要先建一个 ReplicaSet？（积木 1-7，第 5 章会详细讲）
8. `spec` 和 `status` 分别是谁写的？（积木 1-6）
...
> 既然容器才是真正跑东西的东西，为什么 K8s 的最小单位是 **Pod** 而不是**容器**？为什么它非要在一层容器外面再套一层？

我们会讲到：`pause` 容器、共享网络命名空间、`localhost` 互通、共享存储卷、init 容器、sidecar 模式——然后你会明白，Pod 其实不是"容器组"，而是**"一台逻辑上的小机器"**。
