...
> **本章导读**
> - 建议用时：70 分钟（含 20 分钟动手）
> - 前置知识：第 1 ~ 14 章全部
> - 本章是**工具书性质**的一章，不追求一口气读完，而是**需要时回来查**：
>   1. 一份**五层排查模型 + 症状对照表**（覆盖 20 多种异常状态）
>   2. 生产必须补上的四块：**RBAC / 网络策略 / 可观测性 / 备份验证**
>   3. **集群运维**：节点排空、PDB、版本升级、多环境管理
>   4. **一份 40 项的上线前检查清单**
...
## 【积木 15-4】必须补上的四块：权限、网络策略、可观测性、备份
...
> ⚠️ **一个必须知道的前提**：**NetworkPolicy 需要 CNI 插件支持才会生效。**
>
...
1. 五层排查模型是哪五层？每层只问什么问题？（积木 15-1）
2. 排查问题的"黄金三条命令"是什么？（积木 15-1）
3. `Pending` 和 `ContainerCreating` 卡住的常见根因分别有哪些？（积木 15-2）
4. `CrashLoopBackOff` 时为什么必须用 `logs --previous`？（积木 15-2）
5. `Running` 但 `READY 0/1` 和 `RESTARTS` 一直涨，分别是哪一层的问题？（积木 15-2）
6. 排查"Service 连不通"时，为什么第一步是看 `Endpoints`？（积木 15-2）
7. `ping` 不通 ClusterIP 是不是故障？该用什么测？（积木 15-2）
8. `kubectl explain` 比搜文档好在哪？（积木 15-3）
9. **为什么说"不给应用 `cluster-admin`"是底线？**（积木 15-4）
10. 1.24 之后 ServiceAccount Token 有什么重大变化？（积木 15-4）
11. NetworkPolicy 有个"不报错但也不生效"的坑，是什么？（积木 15-4）
12. PSA 的三档分别限制什么？推荐的落地路径是什么？（积木 15-4）
13. **如果只能监控三个 K8s 指标，你选哪三个？为什么？**（积木 15-4）
14. 为什么说"Pod 全绿 ≠ 业务正常"？该补什么？（积木 15-4）
15. 备份的三条铁律是什么？（积木 15-4）
16. `kubectl drain` 和 PDB 是什么关系？PDB 保护哪类中断、不保护哪类？（积木 15-5）
17. 集群升级的几步顺序是什么？为什么要先升控制面？（积木 15-5）
18. Kustomize 多环境管理的核心原则是什么？最常见的反模式是什么？（积木 15-6）
19. 40 项清单里，你觉得哪三项最容易被团队跳过？跳过后果最严重？（积木 15-7）
...
**这份课程的目标不是让你背下所有字段（那些查文档就行），而是让你在第三个层次上思考问题。**

---

*课程正文结束。附录里有**命令速查表、常用 YAML 片段、术语中英对照**，需要时随时回来查。*
