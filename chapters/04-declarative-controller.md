...
6. `resourceVersion` 的作用是什么？为什么 K8s 不直接用悲观锁？（积木 4-6）
7. `kubectl apply` 报 `the object has been modified` 是什么意思？正确做法是什么？（积木 4-6）
8. Deployment 为什么不直接管 Pod，中间要加一层 ReplicaSet？（积木 4-7）
9. `ownerReferences` 和 `labels/selector` 分别表达什么关系？各举一个例子。（积木 4-7）
10. "给用户发一封邮件"适合用声明式表达吗？Job 是怎么把"一次性动作"塞进声明式框架的？（积木 4-9）
