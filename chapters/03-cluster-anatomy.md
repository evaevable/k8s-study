...
2. API Server 的六道处理链里，哪一步负责给 `strategy: RollingUpdate` 这类默认值？（积木 3-2）
3. 为什么说"2 个 etcd 节点比 1 个更不可靠"？3 节点能容忍几个挂？（积木 3-3）
4. Scheduler 的 Filter 和 Score 分别是什么性质的条件？为什么 `LeastAllocated` 是默认打分策略？（积木 3-4）
5. Controller Manager 有 3 个副本，为什么不会出现"3 个都去补 Pod"的混乱？（积木 3-5）
6. kubelet 挂了，节点上正在跑的容器会立刻停止吗？为什么？（积木 3-6、3-8）
7. 节点宕机后，Pod 的"重建"具体发生在哪里？是谁做的？（积木 3-8）
8. 为什么生产集群至少要有 3 个工作节点？只有 1 个会怎样？（积木 3-8）
9. 控制面组件在 kind 集群里为什么是"静态 Pod"而不是普通 Deployment？（积木 3-9）
